...
#FCDenseNet U_Net R2U_Net AttU_Net R2AttU_Net
    # Load model
    model_dir = header.dir_checkpoint + 'FCDenseNetmodel__RANZCR_100_Whole_100_1024v9.1.pth' # -> whole png
    ...
    if os.path.isfile(model_dir):
        ...
        print('[Err] Model does not exist in %s' % (
                    header.dir_checkpoint + "FCDenseNet" + header.filename_model))
        exit()
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(testloader, 0):
            ...
            for k in range(len(data['input'])):
                ...
                if flag_save_PNG:
                    save_dir = save_dir+'/output_inference_segmentation_endtoendFCDenseNet_HT' + header.Whole_Catheter + "/" + 'First_output/Whole'
    ...
    if Hough_transform_process:
        hough_oup = save_dir.replace('/Whole',
                               '/hough')
        mydataset.create_folder(hough_oup)
        filenames = [f for f in os.listdir(save_dir) if not f.endswith('_image.jpg')]
        for files in filenames:
            img_origin = cv2.imread(os.path.join(save_dir, files), cv2.IMREAD_COLOR)
            img_width = img_origin.shape[1]
            img_height = img_origin.shape[0]

            img, img_blue = Probability_Hough_transform(img_origin, thresh, minLineLength, maxLineGap)

            if img == "skip":
                cv2.imwrite(os.path.join(hough_oup, files), img_origin)
            else:
                img = cv2.resize(img, (img_width, img_height))
                cv2.imwrite(os.path.join(hough_oup, files), img)
    ...
    if connected_patch:
        ...
        make_correlation_check(hough_oup)
        print("correlation complete")

        connected_component(hough_oup, oup)
...
def Probability_Hough_transform(img, thresh, minLineLength_param, maxLineGap_param):
    # Read image
    ...
    lines = cv2.HoughLinesP(edges, 1, np.pi / 180, thresh, minLineLength=minLineLength_param,
                            maxLineGap=maxLineGap_param)
    ...
    for line in lines:
        x1, y1, x2, y2 = line[0]
        img_blue = img
        cv2.line(img_blue, (x1, y1), (x2, y2), (255, 0, 0), 3)
    # Show result
